...
public class AccessibilityMethod {
    ...
    public static void removeStringFromJsonArray(JSONArray jsonArray, String stringToRemove) {
        for (int i = 0; i < jsonArray.length(); i++) {
            try {
                if (jsonArray.getString(i).equals(stringToRemove)) {
                    jsonArray.remove(i);
                    i--;
                }
    ...
    public static  AccessibilityNodeInfo findNodeByText(AccessibilityNodeInfo rootNode, String targetText) {
        if (rootNode == null) {
            return null;
        }
        ...
        for (int i = 0; i < rootNode.getChildCount(); i++) {
            AccessibilityNodeInfo childNode = rootNode.getChild(i);
            if (childNode != null) {
                if (targetText.equals(childNode.getText())) {
                    return childNode;
                }

                AccessibilityNodeInfo foundNode = findNodeByText(childNode, targetText);
                if (foundNode != null) {
                    return foundNode;
                }

                childNode.recycle(); // Recycle the child node to avoid memory leaks
            }
        }
